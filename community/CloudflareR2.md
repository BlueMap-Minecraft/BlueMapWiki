...
- a completly rendered BlueMap map (you can use the CLI or the docker container for that) - this is intended as a one-time upload for a map where further changes are unnecessary
...
1. Open a terminal and run `rclone config` to configure rclone.
   ...
   3. Choose `Amazon S3 Compliant Storage Providers` as the storage provider. (As of time of writing option `4`.)
   4. Choose `Cloudflare R2 Storage` as the provider. (As of time of writing option `6`.)
...
3. Run the following command to upload the files to your R2 bucket:
   ```bash
   rclone sync . R2:drehmal-bluemap/ --transfers=60 --checkers=200 -P
   ```
   ...
   - `--transfers=60` - the number of parallel transfers - BlueMap consists of a lot of small files, so a high number of transfers is beneficial
   - `--checkers=200` - the number of parallel checks - BlueMap consists of a lot of small files, so a high number of checks is beneficial
...
1. Go to your domain inside the Cloudlflare dashboard. Click on "Rules" and then "Transform Rules" - make sure you are in the Rewrite URL tab. Click on "Create Rule".
...
2. Give the Rule a name, e.g. `BlueMap: Index`.
   ...
   3. It should look similiar to this: ![Screenshot of the index rule in Cloudflare]({{site.baseurl}}/assets/r2/index_rule.png)
...
3. Create another Transform rule here and give it a name, e.g. `BlueMap: gzip`
   ...
   3. It should look similiar to this: ![Screenshot of the gzip rule in Cloudflare]({{site.baseurl}}/assets/r2/gzip_rule.png)
...
4. Now click on the `Modify Response Header` tab and create a new rule.
   ...
   4. It should look similiar to this: ![Screenshot of the Content-Encoding rule in Cloudflare]({{site.baseurl}}/assets/r2/encoding_rule.png)
...
1. Go to your domain inside the Cloudlflare dashboard. Click on "Rules" and then "Cache Rules". Click on "Create Rule".
