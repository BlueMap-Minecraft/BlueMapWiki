...
For latest and greatest (but possibly unstable) the `latest` tag is the latest release and `master` the latest git commit.
...
- Somehow obtain the configuration folder.
  Easiest is to run `docker run --rm -it -v "$(pwd)/config:/app/config" ghcr.io/bluemap-minecraft/bluemap:v3`.
...
- Start a container to render and host a webserver.
  ...
  docker run --rm -it \
    ...
    -v "$(pwd)/data:/app/data" \
    -v "$(pwd)/web:/app/web" \
    ghcr.io/bluemap-minecraft/bluemap:v3 \
    -r -u -w
  ```
...
services:
  bluemap:
    image: ghcr.io/bluemap-minecraft/bluemap:v3
    restart: always
    command: -r -u -w
    ports:
      - '8100:8100'
